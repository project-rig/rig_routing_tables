...
// inserted.
static inline unsigned int oc_get_insertion_point(merge_t merge)
{
  // TODO
}
...
static inline void oc_upcheck(merge_t merge, int min_goodness)
{
  // Get the point where the merge will be inserted into the table.
  unsigned int insertion_index = oc_get_insertion_point(merge);

  // For every entry in the merge check that the entry would not be covered by
  // any existing entries if it were to be merged.
  for (unsigned int _i = merge.table->size, i = merge.table->size - 1; _i > 0;
       _i--, i--)
  {
    ...
    keymask_t km = merge.table->entries[i].keymask;
    ...
    {
      keymask_t other_km = merge.table->entries[j].keymask;
      ...
      {
        merge_remove(merge, i);  // Remove from the merge
        insertion_index = oc_get_insertion_point(merge);
      }
    }
  }
}
...
static inline void oc_downcheck(merge_t merge, int min_goodness)
{
  // TODO
}
...
static inline merge_t oc_get_best_merge(table_t* table)
{
...
static inline void oc_merge_apply(merge_t m)
{
  // TODO
}
...
static inline void oc_minimise(table_t* table, unsigned int target_length)
{
